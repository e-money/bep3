...
func (suite *AtomicSwapTestSuite) TestCreateAtomicSwap() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			args{
				...
				timeSpan:            types.DefaultSwapTimeSpanMinutes,
				sender:              suite.deputy,
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			// Create atomic swap
			_, err := suite.keeper.CreateAtomicSwapState(suite.ctx, tc.args.randomNumberHash, tc.args.timestamp,
			...
			if tc.expectPass {
				...
				case types.Outgoing:
					// Check coins moved
					suite.Equal(senderBalancePre.Amount.Sub(tc.args.coins[0].Amount), senderBalancePost.Amount)
				...
				expectedSwap :=
					types.AtomicSwap{
						...
						ExpireTimestamp:     suite.ctx.BlockTime().Unix() + tc.args.timeSpan*60,
...
func (suite *AtomicSwapTestSuite) TestClaimAtomicSwap() {
	...
	for i, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			// Create atomic swap
			_, err := suite.keeper.CreateAtomicSwapState(suite.ctx, suite.randomNumberHashes[i], suite.timestamps[i],
				types.DefaultSwapTimeSpanMinutes, sender, expectedRecipient, TestSenderOtherChain, TestRecipientOtherChain,
				tc.args.coins, true)
			suite.NoError(err)
			...
			_, err = suite.keeper.ClaimAtomicSwapState(tc.claimCtx, expectedRecipient, claimSwapID, claimRandomNumber)
			...
			if tc.expectPass {
				...
				case types.Incoming:
					// Check coins moved
					suite.Equal(expectedRecipientBalancePre.Add(tc.args.coins[0].Amount), expectedRecipientBalancePost.Amount)
				...
				suite.Error(err)
				// Check coins not moved
				suite.Equal(expectedRecipientBalancePre, expectedRecipientBalancePost.Amount)
...
func (suite *AtomicSwapTestSuite) getContextPlusMinutes(plusMinutes int64) sdk.Context {
	offset := plusMinutes * 60
...
func (suite *AtomicSwapTestSuite) TestRefundAtomicSwap() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			suite.getContextPlusMinutes(bep3.DefaultSwapTimeSpanMinutes + 1),
			args{
				swapID:    []byte{},
				direction: types.Incoming,
			},
	...
	for i, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			_, err = suite.keeper.RefundAtomicSwapState(tc.refundCtx, sender, refundSwapID)
			...
			if tc.expectPass {
				...
				case types.Outgoing:
					// Check coins moved
					suite.Equal(originalSenderBalancePre.Add(expectedRefundAmount[0].Amount), originalSenderBalancePost.Amount)
				...
				suite.Error(err)
				// Check coins not moved
				suite.Equal(originalSenderBalancePre, originalSenderBalancePost.Amount)
